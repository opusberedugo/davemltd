...
export default function Card2({ 
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex flex-col ${className}`}>
      {imageSrc && (
        <div className="w-full h-48 rounded-xl overflow-hidden mb-6 bg-gray-100 shrink-0">
      ...
      {(title || description) && (
        <div className="flex flex-col flex-1">
          ...
          {description && <p className="text-slate-500 text-base leading-relaxed mb-6 flex-1">{description}</p>}
        </div>
      )}
    </div>
  );
